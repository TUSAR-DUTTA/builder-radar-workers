...
async function runGeoForProject(projectId: string, sources: string[]): Promise<GeoRunResult> {
  ...
  const identityRows = await withDbRetry('runGeoForProject.identity', () => db.execute(sql`
    SELECT
      ...
      (SELECT baseline_version FROM measurement_baselines WHERE project_id = p.id AND status IN ('approved', 'active') ORDER BY created_at DESC LIMIT 1) AS "measurementBaselineVersion",
